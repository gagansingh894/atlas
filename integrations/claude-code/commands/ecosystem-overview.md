...
Follow the instructions in `skills/explore-repo-interface.md` for each accessible repo.
...
## 1. Ecosystem at a Glance
   (table: one row per repo — service, language, deploy, owns, exposes, key deps)

## 2. Dependency Graph (ASCII)
   (directed graph showing service-to-service connections, SYNC/ASYNC on each edge)

## 3. Shared Infrastructure
   (table of resources accessed by >1 repo)

## 4. API & Event Contract Map
   (table of every cross-repo interface: producer, consumer, contract, mechanism, sync?)

## 5. Cross-Repo E2E Flows
   (one sub-section per significant flow crossing ≥2 repos)

## 6. Impact Analysis
   6a. Blast Radius Table
   6b. Single Points of Failure
   6c. Cascade Failure Scenarios (2-3 "what if..." scenarios)

## 7. Data Ownership Map
   (table per service: what stores it owns, who else reads/writes them)

## 8. Coupling & Cohesion Assessment
   (tightly coupled pairs, well-decoupled examples, recommendations)

## 9. Per-Repo Interface Profiles (Raw Data)
   (one sub-section per repo — structured, not prose; source of truth for sections 2-8)
